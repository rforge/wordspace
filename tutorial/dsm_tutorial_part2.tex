...
\begin{frame}
  ...
  \begin{columns}[t]
    ...
    \begin{block}{word forms}
      \begin{itemize}
      \item  \counterpoint{giungere}
      \item  \counterpoint{raggiungere}
      \item  \primary{arrivi}
      \item  \counterpoint{raggiungimento}
...
\begin{frame}
  ...
  \item uniform or ``triangular'' (distance-based) weighting \light{(don't!)}
...
\begin{frame}
  ...
  Contexts range from general/\primary{implict} to specific/\primary{explicit}:
...
\begin{frame}[fragile]
  ...
  DSM objects in \texttt{wordspace} (class \code{dsm}) include marginal frequencies as well as
...
\begin{frame}
  ...
  \item<2-> Preserves sparseness if $x \leq 0$ for all empty cells ($O = 0$)
    \begin{itemize}
    \item combine with \secondary{signed} AM ($x > 0$ for $O > E$, $x < 0$ for $O < E$)
...
\begin{frame}
  ...
  \item<3-> \secondary{sigmoid} transformation as soft binarization
    ...
    f(x) = \tanh x
    \]
  \item<4-> \counterpoint{sparse} AM as \only<5->{\counterpoint{(shifted)} }cutoff transformation
  \end{itemize}
  ...
  \begin{center}
    ...
    \only<beamer:4| handout:0>{\includegraphics[width=8.5cm]{img/transformation_functions_3}}%
    \only<beamer:5-| handout:1>{\includegraphics[width=8.5cm]{img/transformation_functions}}%
  \end{center}
\end{frame}
...
\begin{frame}
  ...
  \item In statistical analysis and machine learning, features are
    usually \primary{centered} and \primary{scaled} so that
    \begin{align*}
      \text{mean} & \quad \mu = 0 \\
  ...
  \begin{columns}[T]
    \begin{column}{60mm}
      ...
      \item \hh{Distance} between vectors $\vu, \vv \in \setR^n$ \so
        ...
        \item $\vv = (v_1, \ldots, v_n)$
        \end{itemize}
      \item \h{Hamming} distance $\dist[0]{\vu}{\vv}$ not very useful for DSM
      \item Extension of the Minkowski \h{$p$-distance} $\dist[p]{\vu}{\vv}$\\ (for $0\leq p\leq 1$)
      \end{itemize}
    \end{column}
    \begin{column}{45mm}
      \includegraphics[width=45mm]{img/2_distance_examples}
...
\begin{frame}
  ...
  \begin{columns}[T]
    \begin{column}{50mm}
      ...
      \item<3-> \h{Minkowski $p$-norm} with\\
        $\dist[p]{\vu}{\vv} = \norm[p]{\vu-\vv}$
      \end{itemize}
    \end{column}
    \begin{column}{50mm}
  ...
  \onslide<3->
  \begin{align*}
    \norm[p]{\vu} &\coloneq \bigl(\abs{u_1}^p + \dots + \abs{u_n}^p\bigr)^{1/p} && \text{ for } 1\leq p \\
    \norm[p]{\vu} &\coloneq \abs{u_1}^p + \dots + \abs{u_n}^p && \text{ for } 0\leq p < 1 \\
    \norm[0]{\vu} &= \# \bigsetdef{i}{u_i \neq 0} & \norm[\infty]{\vu} &= \max \bigset{\abs{u_1}, \ldots, \abs{u_n}}
  \end{align*}
\end{frame}
...
\begin{frame}
  ...
  \item Generalized \h{Jaccard coefficient} = shared features
    ...
    \]
  \item $1 - J(\vu, \vv)$ is a distance \hh{metric} \citep{Kosub:16}
  \item[]
...
\begin{frame}[c]
  ...
  \begin{columns}[c]
    ...
    \end{column}
    \begin{column}{65mm}
      \only<beamer:1| handout:1>{\includegraphics[width=65mm]{img/buy_sell_0_uncentered}}%
      \only<beamer:2| handout:2>{\includegraphics[width=65mm]{img/buy_sell_0_centered}}%
      \only<beamer:3| handout:3>{\includegraphics[width=65mm]{img/buy_sell_1_variance}}%
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}<beamer:1-8| handout:1-3>[c]
  \frametitle{Step 2: Orthogonal projection into optimal subspace}
  %% \framesubtitle{}

  \begin{center}
    \only<beamer:1| handout:0>{\includegraphics[width=7.5cm]{img/buy_sell_2_axis}}%
    \only<beamer:2| handout:1>{\includegraphics[width=7.5cm]{img/buy_sell_2_projection}}%
    \only<beamer:3| handout:0>{\includegraphics[width=7.5cm]{img/buy_sell_3_axis}}%
    \only<beamer:4| handout:2>{\includegraphics[width=7.5cm]{img/buy_sell_3_projection}}%
    \only<beamer:5| handout:0>{\includegraphics[width=7.5cm]{img/buy_sell_4_axis}}%
    \only<beamer:6| handout:0>{\includegraphics[width=7.5cm]{img/buy_sell_4_projection}}%
    \only<beamer:7| handout:0>{\includegraphics[width=7.5cm]{img/buy_sell_5_axis}}%
    \only<beamer:8| handout:3>{\includegraphics[width=7.5cm]{img/buy_sell_5_projection}}%
  \end{center}
\end{frame}


\begin{frame}[c]
  \frametitle{Step 3: Further orthogonal dimensions}
  %% \framesubtitle{}

  \begin{center}
    \only<beamer:1| handout:0>{\includegraphics[width=7.5cm]{img/buy_sell_6_pca}}
    \only<beamer:2| handout:1>{\includegraphics[width=7.5cm]{img/buy_sell_6_pca_labels}}%
  \end{center}
\end{frame}
...
\begin{frame}
  ...
  \item<3-> NB: row vectors should be renormalised after PCA/SVD
    ...
    \item[\hand] also normalise vectors \primary{before} dimensionality reduction
    \end{itemize}
  \end{itemize}
\end{frame}
...
\ungap[1.5]
\begin{Rcode}
\REM{it is customary to omit the centering: SVD dimensionality reduction}
...
\begin{frame}
  ...
  \item<3-> Power-scaling of dimensions: $\sigma^P$ \citep{Caron:01}
    ...
    \item<4-> esp.\ with $P = 0$ to equalize dimensions (\hh{whitening})
    \end{itemize}
  \end{itemize}

  \only<beamer:1| handout:0>{\includegraphics[width=10.5cm]{img/singular_values_1}}%
  \only<beamer:2| handout:0>{\includegraphics[width=10.5cm]{img/singular_values_2}}%
  \only<beamer:3| handout:0>{\includegraphics[width=10.5cm]{img/singular_values_3}}%
  \only<beamer:4| handout:0>{\includegraphics[width=10.5cm]{img/singular_values_4}}%
  \only<beamer:0| handout:1>{%
    \begin{tabular}{c@{\hspace{4mm}}c}
      \includegraphics[width=5cm]{img/singular_values_1} &
      \includegraphics[width=5cm]{img/singular_values_2} \\
...
effect -0.383  0.3704
\end{Rout}
\REM{power-scaling can also be applied post-hoc}
> sigma <- attr(TT2, "sigma")       \REM{singular values}
> scaleMargins(TT2, cols=sigma^0.5) \REM{\(P = 1/2\)}
> scaleMargins(TT2, cols=sigma)     \REM{unscaled (\(P = 1\))}
\end{Rcode}
\end{frame}
...
\begin{frame}
  \frametitle{Practice}
  \begin{itemize}
  \item How many different models can you build from \code{\small DSM\_VerbNounTriples\_BNC}?
  ...
  \item Code examples for this part show additional options
    ...
    \item[]
    \end{itemize}
  \item Download practice session (\code{\small part2\_input\_formats.R})\\
    \so different ways of loading your own co-occurrence data
  \item Build real-life DSMs from pre-compiled co-occurrence data
    \begin{itemize}
    \item \primary{\href{http://wordspace.collocations.de/doku.php/course:material\#dsms_based_on_the_english_wikipedia}{http://wordspace.collocations.de/doku.php/course:material}}
    \item also download R script with instructions (\code{part2\_exercise.R})
\end{itemize}
  \end{itemize}
\end{frame}
