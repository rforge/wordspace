...
*  add (manual) deprecation warnings for `rbind`, `cbind` and `merge`
    - functions will be re-designed in a future release or removed completely (`merge`)
    - `cbind` hasn't been implemented yet anyway
...
*  function `read.dsm.matrix` for reading in a pre-compiled DSM matrix in various formats
    - in particular, try to find efficient way to read word2vec format (and possibly GloVE, which is word2vec without the header)
...
*  implement `cbind.dsm()`
*  `rbind` and `cbind` should support _intersecting_ and _expanding_ combinariona (for which `merge` was designed, but only supports partially yet)
...
* `merge` should be rewritten to _add_ DSMs from different corpora with the same targets and features (also as _intersecting_ or _expanding_ merge)
