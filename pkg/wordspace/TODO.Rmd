...
*  `as.distmat()` method for `dist` objects, which converts them to full symmetric matrix
    - should be as simple as `as.distmat(as.matrix(x))`
...
*  investigate memory overhead shown by benchmarks
    ...
    - overhead in `read.dsm.ucs()` has considerable overhead in R-level implementation of readMM
    ...
    - perhaps overhead can be reduced without the intermediate dgTMatrix (but need at least one complete copy for reording triplets into CSR)
    - non-zero counts have unnecessary memory overhead, in particular in `dsm()` constructor
    - implemented as `rowSums(M != 0)` / `colSums(M != 0)`
    - logical vectors are stored as `int`'s, hence the logical matrix `(M != 0)` needs considerable storage
    - even though `rowSums` and `colSums` have C-level method implementations for a `lgCMatrix`, it is converted internally to a `dgCMatrix` with huge memory overhead
    - should implement our own `rowsNNzero` and `colsNNzero` functions for sparse (and dense) matrices; first check which other operations may need to be supported
