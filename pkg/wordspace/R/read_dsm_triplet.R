read.dsm.triplet <- function (filename, freq=FALSE, value.first=FALSE, sep="\t", quote="", nmax=-1, sort=FALSE, encoding=getOption("encoding"), verbose=FALSE) {
  ...
  if (verbose) cat(sprintf("%.2fM items\n", length(triplets$l1) / 1e6))
